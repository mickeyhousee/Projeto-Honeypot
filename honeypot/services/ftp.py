...
def handle_client(client_socket, addr):
    ...
    try:
        ...
        log_event("FTP", addr[0], timestamp, data)
        send_email_alert(
            subject=f"FTP Honeypot Connection from {addr[0]}",
            message=f"Connection at {timestamp}\nData: {data}"
        )
    except Exception as e:
        log_event("FTP", addr[0], timestamp, f"Error: {e}")
    finally:
        client_socket.close()

def run_ftp_honeypot(port=2121):
